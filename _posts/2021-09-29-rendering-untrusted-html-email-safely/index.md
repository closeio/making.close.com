...
We launched our ability to display in 2014, however the technical side of it is still very relevant and (I think) interesting since our approach is not one that I've seen widely used.
...
- Apply a [`Content-Security-Policy`](https://content-security-policy.com/) instructing the iframe document to avoid executing any JavaScript whatsoever. (If we used a hosted page with `src` instead of `srcdoc`, this would be done with an HTTP Header instead).
...
This attribute is the heart of our security model. By adding this attribute, it adds a whole bunch of restrictions to the iframe document, including preventing any JavaScript from running! Learn more about what's blocked [here](https://html.spec.whatwg.org/multipage/iframe-embed-object.html#attr-iframe-sandbox) and [here](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe). The `sandbox` attribute allows you to opt back in specific behaviors that are blocked by default. We only allow:
  ...
  - `allow-same-origin` – We need this for our web app to detect the height the of iframe's content. This would be dangerous (and defeat the purpose of `sandbox`) if we also allowed scripts to run by also including `allow-scripts`, however it's harmless on its own.
...
One can invision additional enhancements where we _would_ want to parse and modify the email HTML before rendering it. One example would be if we wanted to try to rewrite and proxy all image URLs ([similar to Gmail](https://gmail.googleblog.com/2013/12/images-now-showing.html)) to protect our user's privacy.
...
- We need to consider how composing email works too. We needed a WYSIWYG editor for HTML Email that also prevents any XSS. We currently use [Froala Editor](https://froala.com/wysiwyg-editor/) for this, after having testing many editors for security.
