...
We use mongo to store a *lot* of our data. It was hip in 2013, and still houses terabytes of our customers’ data. [Mongoengine](https://github.com/MongoEngine/mongoengine) (a mongodb ORM) is even maintained by one of [us](http://github.com/wojcikstefan)! What started with a flaky test during our py3 upgrade turned into a deep dive into how python dictionaries intersect with `mongoengine`, `pymongo`, and `bson`.
...
Yeah, we’re getting to that. But first, let’s answer this question: *why does python dictionary ordering impact mongo at all?* Because in mongo, objects are ordered. So :

```python
{'a': 'a', 'b': 'b'} != {'b': 'b', 'a': 'a'}
```

*Ok… and?* Yodawg because in mongo, you put documents inside your documents, i.e.
...
- Different versions of python will produce different key orders (which according to mongo isn’t the same value)
